package mod.arcomit.emberthral.core.filter.event;
...
import mod.arcomit.emberthral.core.filter.gui.FilterButton;
import mod.arcomit.emberthral.core.filter.gui.PageButton;
    ...
    public static void onGuiInit(ScreenEvent.Init.Post event) {
        ...
        if (!init){
            ...
            registryAccess.registry(Filter.REGISTRY_KEY).get().forEach(filter -> {
                ...
                if (tab == null) {
                    EmberthralMod.LOGGER.warn("Filter：[{}] has no tab to be filtered!", registryAccess.registryOrThrow(Filter.REGISTRY_KEY).getResourceKey(filter).get().location());
                    return;
                }
                filter.loadItems();
